...
def silence_detect(audio_filename, noise_tolerance = 0.12, silence_duration = 0.5, max_duration = None):
    '''
...
def silence_remove(audio_filename, output_filename = None):
    '''
    ...
    intervals = silence_detect(audio_filename)    
    for idx in range(0,len(intervals)):
        start = intervals[idx][0]
        end = intervals[idx][1]
        duration = end - start        
                        
        trim_command = "sox {0} {1}{2}.wav trim {3} {4}"
...
silence_remove(audio_filename, out_filename)
